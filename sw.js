const CACHE = 'powerdash-v125-perf';
// v125: slower tick, pause when hidden, skip rain/compass work, memoize HTML rewrite.
...
function patchJs(html){
    ...
    html=html.replace(
        ...
        'function buildRain(intensity){return;'
    );
    html=html.replace(
...
function restyleDashboard(html){
    ...
    if(html.indexOf('bg-paused')<0){
        html=html.replace('</style>', 'body.bg-paused .fdot,body.bg-paused .pulse-dot,body.bg-paused .rdrop{animation-play-state:paused!important}</style>');
    }
